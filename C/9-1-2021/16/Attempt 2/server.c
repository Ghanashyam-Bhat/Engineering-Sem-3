...
{
	NODE *cur1=h->head,*cur2=he->head,*pres=final->head;
	NODE* temp;
	while(cur1 && cur2)
	{
		printf("**\n");
		if(cur1->poly.power>cur2->poly.power)
		{
			printf("1\n");
			operation(final,n);
			if(final->head)
		...
		else if(cur1->poly.power<cur2->poly.power)
		{
			printf("2\n");
			if(final->head)
			{
		...
		else
		{
			...
			printf("3\n");
			temp = (NODE*)malloc(sizeof(NODE));
			temp->poly.coeff = cur1->poly.coeff+cur2->poly.coeff;
			temp->poly.power = cur1->poly.power;
			cur1=cur1->link;
			cur2=cur2->link;
			if(final->head)
				pres->link2 = temp;
